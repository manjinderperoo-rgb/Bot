...
async def process_tab(tab_id, page, url, msg, dm_selector):
    """Process a single tab: navigate, wait, send one message with retries"""
    max_retries = 2
    for retry in range(max_retries):
        try:
            ...
            if not page.locator(dm_selector).is_visible():
                print(f"Tab {tab_id}: Selector not visible, attempting Enter to clear.")
                try:
            ...
            await asyncio.sleep(0.6)  # Brief settle after send
            return True
        except Exception as e:
...
async def main():
    """Windows main function with infinite batched looping"""
    parser = argparse.ArgumentParser(description="Instagram DM Infinite Batch Looper for Windows")
    ...
    batch_size = min(10, len(thread_urls))
    print(f"Using {batch_size} persistent tabs to loop over {len(thread_urls)} threads infinitely. Press Ctrl+C to stop.")
